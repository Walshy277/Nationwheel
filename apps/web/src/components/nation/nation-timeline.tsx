  ...
  return (
    <Panel className="p-4 sm:p-5">
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-xl font-bold text-zinc-50">Timeline</h2>
      ...
      <ol className="relative grid gap-3 border-l border-white/15 pl-4">
        {entries.map((entry, index) => (
          <li key={`${entry.title}-${index}`} className="relative">
            <span className="absolute -left-[1.58rem] top-1.5 h-3 w-3 rounded-full border border-emerald-100 bg-[color:var(--panel-strong)] shadow-[0_0_0_4px_rgba(93,230,189,0.12)]" />
            <div className="rounded-lg border border-white/10 bg-black/20 p-3 sm:p-4">
              ...
              <h3 className="text-base font-bold text-zinc-50 sm:text-lg">
                {entry.title}
              </h3>

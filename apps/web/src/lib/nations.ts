...
export async function listNationSummaries() {
  ...
  const dbNations: NationSummary[] = nations.map((nation) => {
    const normalized = normalizeNation({
      ...
      overview: nation.overview,
      area: nation.area ?? undefined,
      geoPoliticalStatus: nation.geoPoliticalStatus ?? undefined,
      block: nation.block ?? undefined,
      culture: nation.culture ?? undefined,
      hdi: nation.hdi ?? undefined,
